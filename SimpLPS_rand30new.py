...
def Orthogonalize_left2right(mps, chi):
    ...
    for i in range(0, len(mps) - 1):
        ...
        u, lm, v = u.to(dtype=type, device='cuda'), lm.to(dtype=type, device='cuda'), v.to(dtype=type, device='cuda')
        bdm = min(chi, len(lm))
        u = u[:, :bdm]
        lm = tc.diag(lm[:bdm])
        v = v[:bdm, :]
...
'''随机态和密度矩阵'''
f = open('/home/LiWenjun/code/Data/target_state/Rand_large/chain30/state30_normal_1_3_0.pr', 'rb')
rhostate = pickle.load(f)
f.close()
for i in range(len(rhostate)):
    rhostate[i] = tc.from_numpy(rhostate[i]).cuda()
...
def SimpleMPS(train_dataset, para=None):
    ...
    dic = {'trainloss_List': trainloss_List, 'trainloss_log': trainloss_log,
           'testloss_List': testloss_List, 'testloss_log': testloss_log, 'fyang': fide1}
    f = open('/home/LiWenjun/code/Result/Rand_large/chain30/normal_1_3_0/%d_%d_chi%d_miu%d.txt' % (
        para['measure_train'], para['sample_num'], para['chi'], 230), 'wb')
    pickle.dump(dic, f)
    f.close()
...
def run():
    ...
    para['epoch'] = 500
    para['batch'] = 5000  # iteration=30-40
    para['lr'] = 5 * 1e-3

    para['measure_train'] = 100
    para['sample_num'] = 30000
    para['device'] = 'cuda'  # 'cpu'
    para['dtype'] = tc.complex128

    xy = np.loadtxt('/home/sdb1/LiWenjun/code/Data/Rand_large/chain%d/normal_1_3_0/%d_%d.txt' % (
        para['num_f'], para['measure_train'], para['sample_num']), delimiter=',', dtype=np.float64, skiprows=1)
    f = open('/home/sdb1/LiWenjun/code/Data/Rand_large/chain%d/normal_1_3_0/%d_%d.txt' % (
